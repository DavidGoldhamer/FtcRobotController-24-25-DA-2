...
public class colormoving extends LinearOpMode {
  ...
  public void runOpMode() {
    ...
    while (opModeIsActive() || opModeInInit()) {
      ...
      if (gamepad1.a) {
          tf_ = !(Math.min(Math.max(Math.abs(myBoxFit.center.x), widthxforlocal - 200), widthxforlocal + 200) == widthxforlocal - 200 || Math.min(Math.max(Math.abs(myBoxFit.center.x), widthxforlocal - 200), widthxforlocal + 200) == widthxforlocal + 200) && Math.abs(myBoxFit.center.y) > heightyforlocal + 150;
        speedformot = 0.2;
        rotatedquads();
        quads();
      }
